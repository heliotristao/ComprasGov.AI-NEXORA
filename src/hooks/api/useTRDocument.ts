...
export function useTRDocument(documentoId: number) {
  ...
  } = useQuery<DocumentoTR>({
    queryKey: ["tr-documento", documentoId],
    queryFn: async () => {
      const response = await api.get(`/api/v1/tr/${documentoId}`)
  ...
  const updateMutation = useMutation({
    mutationFn: async (dados: Record<string, any>) => {
      const response = await api.put(`/api/v1/tr/${documentoId}`, {
        dados
      })
      return response.data
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["tr-documento", documentoId] })
    }
  })
  ...
  const validarMutation = useMutation({
    mutationFn: async () => {
      const response = await api.get(`/api/v1/tr/${documentoId}/validar`)
      return response.data
    }
  })
  ...
  const gerarCampoMutation = useMutation({
    mutationFn: async (params: {
      ...
      const response = await api.post(`/api/v1/tr/${documentoId}/gerar-campo`, params)
      return response.data
    }
  })
  ...
  const aceitarIAMutation = useMutation({
    mutationFn: async (params: {
      ...
      const response = await api.post(
        `/api/v1/tr/${documentoId}/aceitar-ia/${params.campo_id}`,
        {
