...
export async function GET(request: NextRequest, { params }: RouteContext) {
  const targetUrl = buildPlanningApiUrl(`/processes/${params.id}`)
  ...
  try {
    ...
    console.error(`Erro ao carregar processo ${params.id}`, error)
    return proxyErrorResponse("Não foi possível carregar os detalhes do processo solicitado.")
  }
}
